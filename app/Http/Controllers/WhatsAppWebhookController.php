...
class WhatsAppWebhookController extends Controller
{
    ...
    private function processCommand($platform, $senderId, $text)
    {
        ...
        elseif (str_starts_with($lowerText, 'dev pay')) {
            ...
            $rawPin = $escrowService->lockEscrowFunds($escrow, $totalExpected);

            // Fetch the updated escrow to verify
            $updatedEscrow = EscrowTransaction::find($escrow->id);
            if ($updatedEscrow && $updatedEscrow->status === 'funded_locked') {
                $pinText = is_string($rawPin) ? $rawPin : '(Check Vendor App)';
                $this->sendMessage($platform, $senderId, "✅ *BYPASS SUCCESS!*\n\nStatus is now 'funded_locked'.\nTo continue testing, your Delivery PIN is: *$pinText*");
            }
        }
        else {
