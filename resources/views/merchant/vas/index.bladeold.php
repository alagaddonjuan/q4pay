...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="flex flex-col gap-2" id="vas-tabs">
                ...
                <button onclick="switchTab('data')" id="tab-data" class="vas-tab-btn flex items-center gap-4 p-4 rounded-xl text-left transition-all hover:bg-slate-50 text-slate-600 border border-transparent">
                    <div class="size-10 rounded-full bg-slate-100 flex items-center justify-center shrink-0">
                        <i class="las la-wifi text-xl"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-base text-slate-700">Data Bundles</h4>
                        <p class="text-xs text-slate-400">Direct data top-up</p>
                    </div>
                </button>
                ...
                <button onclick="switchTab('internet')" id="tab-internet" class="vas-tab-btn flex items-center gap-4 p-4 rounded-xl text-left transition-all hover:bg-slate-50 text-slate-600 border border-transparent">
                    <div class="size-10 rounded-full bg-slate-100 flex items-center justify-center shrink-0">
                        <i class="las la-globe text-xl"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-base text-slate-700">Internet</h4>
                        <p class="text-xs text-slate-400">Smile, Spectranet</p>
                    </div>
                </button>
    ...
    <div class="lg:col-span-8">
        ...
        <div id="form-data" class="vas-form box bg-white rounded-2xl shadow-sm border border-slate-200 hidden overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h3 class="text-lg font-bold text-[#003366] flex items-center gap-2"><i class="las la-wifi text-[#D20103]"></i> Buy Data Bundles</h3>
            </div>
    ...
    <option value="IBEDC">Ibadan Electric (IBEDC)</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select id="tv_biller" required class="w-full text-sm bg-white border border-slate-200 rounded-lg px-4 py-3 focus:border-[#003366] outline-none">
                                <option value="CW-DSTV">DSTV</option>
                                <option value="CW-GOTV">GOTV</option>
                                <option value="CW-STARTIMES">Startimes</option>
                                <option value="SHOWMAX">Showmax</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-[#003366] mb-2">Package Code (Item ID)</label>
                            <input type="text" id="tv_package_code" required placeholder="E.g. DSTV-Premium" class="w-full text-sm bg-white border border-slate-200 rounded-lg px-4 py-3 focus:border-[#003366] outline-none font-mono">
                        </div>
                        <div class="md:col-span-2">
        ...
        <div id="form-internet" class="vas-form hidden box bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            ...
            <div class="p-10 text-center">
                <div class="size-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="las la-tools text-4xl text-slate-400"></i>
                </div>
                <h4 class="text-xl font-bold text-slate-700 mb-2">Coming Soon</h4>
                <p class="text-slate-500">Internet data subscriptions are currently undergoing maintenance.</p>
            </div>
        </div>
        ...
        <div id="form-exams" class="vas-form hidden box bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-10 text-center">
                ...
                <h4 class="text-xl font-bold text-slate-700 mb-2">Coming Soon</h4>
                <p class="text-slate-500">Exam PIN purchases (WAEC, NECO) will be available shortly.</p>
            </div>
        </div>
    </div>
</div>
...
<script>
        ...
        body: JSON.stringify({
            ...
            if (!response.ok) {
                ...
                if(Array.isArray(res.data)) {
                    ...
                    // Fallback for mocked response testing
                    planSelect.innerHTML += `<option value="DATA-1GB" data-price="500">1GB Daily - ₦500</option>`;
                    planSelect.innerHTML += `<option value="DATA-5GB" data-price="1500">5GB Weekly - ₦1,500</option>`;
                }
    ...
    async function verifyTv(e) {
        ...
        try {
            // Note: Currently calling the old validateMeter as a hack since 9psb validates via the same endpoint for some services
            // If the provider has a different validate endpoint, it can be swapped. But since there's no validate in TvSubscriptionController,
            // we will simulate the validation for now or allow them to just pass through
            const meterNumber = document.getElementById('tv_smartcard_number').value;
            document.getElementById('tv_customer_name').innerText = 'Validated Smartcard (' + meterNumber + ')';
            ...
            alertBox.className = 'p-4 rounded-lg text-sm font-bold mb-4 bg-red-50 text-red-700 border border-red-200';
            alertBox.innerHTML = '<i class="las la-wifi text-lg mr-1"></i> Error.';
            alertBox.classList.remove('hidden');
